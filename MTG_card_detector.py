...
def four_point_transform(image, poly):
    """
    A perspective transform for a quadrilateral.
    """
    pts = np.zeros((4, 2))
...
def line_intersection(x_1, x_2, x_3, x_4, y_1, y_2, y_3, y_4):
    """
    ...
    """
    if (x_1 - x_2) * (y_3 - y_4) == (y_1 - y_2) * (x_3 - x_4):
        # parallel lines
        xis = np.nan
        yis = np.nan
    else:
        xis = ((x_1 * y_2 - y_1 * x_2) * (x_3 - x_4) -
               (x_1 - x_2) * (x_3 * y_4 - y_3 * x_4)) / \
              ((x_1 - x_2) * (y_3 - y_4) - (y_1 - y_2) * (x_3 - x_4))

        yis = ((x_1 * y_2 - y_1 * x_2) * (y_3 - y_4) -
...
def simplify_polygon(in_poly, tol=0.05, maxiter=None, segment_to_remove=None):
    ...
    while len_poly > 4:
        ...
        if segment_to_remove is not None:
            ...
            k = np.argmin(d_in)
        if d_in[k] < tol * d_tot:
            xis, yis = line_intersection(x_in[(k-1) % len_poly], x_in[(k) % len_poly],
                                         x_in[(k+1) %
                                              len_poly], x_in[(k+2) % len_poly],
                                         y_in[(k-1) %
                                              len_poly], y_in[(k) % len_poly],
                                         y_in[(k+1) % len_poly], y_in[(k+2) % len_poly])
            x_in[k] = xis
            y_in[k] = yis
...
def generate_quad_candidates(in_poly):
    """
    Generates a list of bounding quadrilaterals for a polygon, using all possible combinations
    of four intersection points derived from four extended polygon segments.
    The number of combinations increases rapidly with the order of the polygon, so simplification
    should be applied first to remove very short segments from the polygon.
    """
    ...
    (x_s, y_s) = order_polygon_points(x_unsorted, y_unsorted)
    quads = []
    len_poly = len(x_s)
    for i in range(0, len_poly):
        for j in range(i+1, len_poly):
            (x_ij, y_ij) = line_intersection(x_s[i % len_poly],
                                             x_s[(i+1) % len_poly],
                                             x_s[j % len_poly],
                                             x_s[(j+1) % len_poly],
                                             y_s[i % len_poly],
            ...
            for k in range(j+1, len_poly):
                (x_jk, y_jk) = line_intersection(x_s[j % len_poly],
                                                 x_s[(j+1) % len_poly],
                                                 x_s[k % len_poly],
                                                 x_s[(k+1) % len_poly],
                                                 y_s[j % len_poly],
                ...
                for l in range(k+1, len_poly):
                    (x_kl, y_kl) = line_intersection(x_s[k % len_poly],
                                                     x_s[(k+1) % len_poly],
                                                     x_s[l % len_poly],
                                                     x_s[(l+1) % len_poly],
                                                     y_s[k % len_poly],
                    ...
                    (x_li, y_li) = line_intersection(x_s[l % len_poly],
                                                     x_s[(l+1) % len_poly],
                                                     x_s[i % len_poly],
                                                     x_s[(i+1) % len_poly],
                                                     y_s[l % len_poly],
...
class MTGCardDetector:
    ...
    def segment_image(self, full_image):
        ...
        for im_seg, card_cnt, bquad in zip(warped_list, card_list, bounding_poly_list):
            self.candidate_list.append(CardCandidate(
                im_seg, card_cnt, bquad, bquad.area/image_area))

        """double_segmented = []
        for ip, iPoly in enumerate(self.bounding_poly_list):
            for jp, jPoly in enumerate(self.bounding_poly_list):
        ...
        for ids in reversed(double_segmented):
            del self.warped_list[ids]
            del self.card_list[ids]
            del self.bounding_poly_list[ids]
         """
        # return warped_list, card_list, bounding_poly_list

    def phash_compare(self, im_seg):
        """
        Runs perceptive hash comparison between given image and the (pre-hashed) reference set.
        """

        card_name = 'unknown'
        d_0_dist = np.zeros(4)
        d_0 = np.zeros((len(self.ref_img_clahe), 4))
        for j in range(4):
            im_seg_rot = rotate(im_seg, j * 90)
            phash_im = imagehash.phash(PILImage.fromarray(
                np.uint8(255 * cv2.cvtColor(im_seg_rot, cv2.COLOR_BGR2RGB))), hash_size=32)
            ...
            if (d_0_dist[j] > self.hash_separation_thr and np.argmax(d_0_dist) == j):
                card_name = self.ref_filenames[np.argmin(d_0[:, j])].split('.jpg')[0]
                break
        return card_name
    ...
    def run_recognition(self, image_index):
        ...
        for candidate in self.candidate_list:
            ...
            bounding_poly = Polygon([[x, y] for (x, y) in zip(
                bquad_corners[:, 0], bquad_corners[:, 1])])
